'''
Heme cooperativity analysis module.

This module provides functionality for analyzing heme-heme interactions
and cooperativity effects in electron transfer chains. While it can be used
standalone via command line, within BioDC it is integrated through the
HemeCooperativityAnalyzer class of cooperativity_analyzer.py.

Command line usage is preserved for standalone analysis and testing.
'''
...
def read_energy_matrix(filename, energy_shift=0, interaction_scale=1.0):
    """
    Read energy matrix from file. Supports both BioDC format and comma-separated format.

    BioDC format:
        238 241 244 247 250
        -0.537534 0.032749 0.004072 0.004317 0.007611
        ...
    ...
    is_biodc_format = not first_line.startswith('[')
...
def create_comparison_plot(sequential_results, geometric_results, directory):
    ...
    try:
        ...
        # Create x-axis labels with 1' for the highest number
        x_labels = [str(i) if i < num_hemes else "1'" for i in heme_positions]
        ...
        for ax in (ax1, ax2):
            ...
            ax.set_xticklabels(x_labels)  # Set the custom x-axis labels
